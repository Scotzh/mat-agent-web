...
IMAGE_DIR = os.path.join(PROJECT_ROOT, "cache", "temp_images")
HTML_DIR = os.path.join(PROJECT_ROOT, "cache", "temp_3d")
...
STRUCTURE_INFO_FILE = os.path.join(PROJECT_ROOT, "cache", "structure_info.json")
    ...
    if os.path.exists(STRUCTURE_INFO_FILE):
        try:
            with open(STRUCTURE_INFO_FILE, "r", encoding="utf-8") as f:
                ...
                STRUCTURE_QUEUE = deque(
                    queue_ids[-MAX_STRUCTURES:]
                    if len(queue_ids) > MAX_STRUCTURES
                    else queue_ids
                )
...
def _save_structure_info():
    ...
    try:
        ...
        data = {"info": serializable_info, "queue": list(STRUCTURE_QUEUE)}
...
def view_structure(id):
    ...
    if id not in STRUCTURE_INFO:
        return (
            f"<h3>结构已失效 (ID: {id})</h3><p>可能是加载了太多新结构，旧结构已被清理。</p>",
            404,
        )
...
def download_cif(id):
    ...
    return send_file(
        sio, as_attachment=True, download_name=f"{info['reduced_formula']}.cif"
    )
            ...
            "sites": [
                {
                    ...
                    "coords": [round(c, 4) for c in site.frac_coords],
                }
